...
def robot_html(vinCode):
    ...
    for i in range(try_count):
        try:
            response_1 = requests.get(home_url, timeout=timeout, verify=False)
    ...
    for i in range(try_count):
        try:
            response_2 = requests.post(post_url, timeout=timeout, data=payload, cookies=cookies, headers=headers_template, verify=False)
        except requests.exceptions.ConnectTimeout:
            continue
        except requests.exceptions.ReadTimeout:
            continue
